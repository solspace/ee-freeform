...
class Freeform_SubmissionElementType extends BaseElementType
{
    ...
    public function getSources($context = null)
    {
        ...
        $formsService = craft()->freeform_forms;
        ...
        foreach ($formsService->getAllForms() as $form) {
            if (!is_null($allowedFormIds) && !in_array($form->id, $allowedFormIds)) {
                continue;
            }
    ...
    public function getAvailableActions($source = null)
    {
        ...
        if (PermissionsHelper::checkPermission(PermissionsHelper::PERMISSION_SUBMISSIONS_MANAGE)) {
            ...
            $setStatusAction = craft()->elements->getAction('Freeform_SetStatus');
    ...
    public function modifyElementsQuery(DbCommand $query, ElementCriteriaModel $criteria)
    {
        ...
        if ($criteria->order) {
            ...
            preg_match_all("/(?:[a-zA-Z0-9]+\.)?([a-zA-Z0-9_]+)/", $orderString, $matches);
    ...
    {
        $fields = array_keys($this->getFieldColumns());
    ...
    private function getFieldColumns($formId = null)
    {
        ...
        if (is_null(self::$fieldCache) || !isset(self::$fieldCache[$formId])) {
            $fieldColumns = [];

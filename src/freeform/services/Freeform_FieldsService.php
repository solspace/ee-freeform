...
class Freeform_FieldsService extends BaseApplicationComponent
{
    ...
    {
        if (is_null(self::$fieldCache) || !self::$allFieldsLoaded) {
    ...
    {
        if (is_null(self::$fieldHandleCache)) {
            $results = craft()
                ->db
                ->createCommand()
                ->select("id, handle")
    ...
    {
        if (is_null(self::$fieldCache) || !isset(self::$fieldCache[$id])) {
            if (is_null(self::$fieldCache)) {
                self::$fieldCache = [];
            }
